'''
Guilherme está entediado nessa quarentena e começa a brincar com a criação de substrings com a ordem de seus caracteres invertidos, de tamanhos variados, a partir de uma string inicial. Sabendo disso, você entra em contato com Guilherme e faz q
 consultas sobre as possíveis substrings que podem ser geradas. Cada consulta contém dois números inteiros l
 e r
...
, mas que sejam impressos em ordem inversa, obtendo-se srsr−1…sl−1sl
.
...
Para cada consulta, imprima uma única linha - a substring resultante slsl+1…sr+1sr
 com seus caracteres em ordem inversa, isto é, deve-se imprimir a sequência de caracteres srsr−1…sl−1sl
. 
'''
# Função para processar cada consulta e gerar a substring invertida
def gerar_substring_invertida(l, r, s):
...
for _ in range(q):
    ...
    # Geramos e imprimimos a substring invertida correspondente
    print(gerar_substring_invertida(l, r, s))
